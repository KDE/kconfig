...
QString warningProlog(const auto &deviceInterface, int line)
{
    ...
    return u"KConfigIni: In file %2, line %1:"_s.arg(line).arg(deviceInterface->id());
}
} // anonymous namespace
    ...
    for (int i = 0; i < l; i++, r++) {
        if (str[i] != '\\') {
            *r = str[i];
            ...
            default:
                *r = '\\';
                qCWarning(KCONFIG_CORE_LOG).noquote() << warningProlog(mDeviceInterface, line) << u"Invalid escape sequence: «\\%1»"_s.arg(str[i]);
            }
        }
    }
