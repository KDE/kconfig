...
#define META(x) QKeySequence(QKeyCombination(Qt::META | Qt::Key_##x))
...
static KStandardShortcutInfo g_infoStandardShortcut[] = {
    ...
    {Open,
     ...
     QList<QKeySequence>{QList{CTRL(O)}},
     QList<QKeySequence>(),
     false,
     Category::File},
...
#if defined(Q_OS_MACOS)
     QList<QKeySequence>{Qt::Key_Back},
#else
...
#if defined(Q_OS_MACOS)
     QList<QKeySequence>{Qt::Key_Forward},
#else
...
#if defined(Q_OS_MACOS)
     QList<QKeySequence>{META(G)}, // Ctrl+G on macos
#else
