...
int main(int argc, char **argv)
{
    ...
    const bool dumpEntries = parser.isSet(QStringLiteral("dump"));
        ...
        for (const QString &groupName : groups) {
            ...
            for (const QString &key : keys) {
                if (dumpEntries) {
                    ...
                    // this entry was added and then deleted in the default config files
                    fprintf(stdout, "  %s (default): <deleted>\n", qPrintable(key));
                }
            }
